...
- Fast mode: PyMuPDF with multi-strategy table detection (~70-80% accuracy)
- Accurate mode: IBM Docling with TableFormer AI (~93.6% accuracy, slower)
"""
...
def extract_pdf_accurate(pdf_path: str, show_progress: bool = False) -> str:
    """
    Extract PDF to markdown using Docling with accurate table mode.

    Uses IBM's TableFormer AI model for ~93.6% table extraction accuracy.
    Much slower than fast mode (~2-3 sec/page).
    ...
    if not check_docling_models():
        print("First run: downloading Docling AI models (one-time setup, ~2-3 minutes)...",
              file=sys.stderr)
    ...
    # Use ACCURATE mode for best table extraction
    try:
    ...
    if show_progress:
        print(f"Processing PDF with Docling (accurate mode, ~2-3 sec/page)...", file=sys.stderr)
...
def extract_pdf_docling(
    ...
    if show_progress:
        print(f"Processing PDF with Docling (accurate mode, ~1 sec/page)...", file=sys.stderr)

    # Configure pipeline for accurate tables + image extraction
    pipeline_options = PdfPipelineOptions(
        do_table_structure=True,
        generate_picture_images=output_dir is not None,  # Only extract images if we have output dir
    ...
    if output_dir:
        ...
        for i, (element, _level) in enumerate(result.document.iterate_items()):
            if hasattr(element, 'image') and element.image is not None:
                img_path = output_path / f"figure_{i:04d}.png"
                element.image.pil_image.save(str(img_path))
                image_paths.append(str(img_path))

        if show_progress and image_paths:
            print(f"Extracted {len(image_paths)} images at {images_scale}x resolution", file=sys.stderr)
    ...
    for img_path in image_paths:
        md = md.replace(
            "<!-- image -->",
            f"![Figure](images/{Path(img_path).name})",
            1
        )
...
def extract_pdf_to_markdown(pdf_path: str, accurate: bool = False, show_progress: bool = False) -> str:
    """
    Extract PDF to markdown with configurable accuracy/speed trade-off.

    Args:
        pdf_path: Path to the PDF file
        accurate: If True, use Docling AI (~93.6% accuracy, slow).
                  If False, use PyMuPDF (~70-80% accuracy, fast).
        show_progress: Whether to show progress output
    ...
    """
    if accurate:
        return extract_pdf_accurate(pdf_path, show_progress)
    else:
...
def extract_images(pdf_path: str, output_dir: str, show_progress: bool = False) -> list:
    ...
    if show_progress and extracted:
        print(f"Extracted {len(extracted)} images", file=sys.stderr)
